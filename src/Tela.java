...
public class Tela extends JFrame {
    ...
    private void resetarMatriz(int nivel) {
        ...
        this.campo = new CampoMinado.CampoDeJogo(nivel);
    ...
    private void renderizaQuadrados(int linhas, int colunas) {
        this.botao = new CampoMinado.JButtonQuadrado[linhas][colunas];
        this.campo.toString();
        for (int i = 0; i < linhas; i++) {
            for (int j = 0; j < colunas; j++) {

                this.botao[i][j] = new CampoMinado.JButtonQuadrado(campo.getQuadrado(i, j), this);
        ...
        try {
            ...
            for (int i = 0; i < this.campo.getLinhas(); i++) {
                for (int j = 0; j < this.campo.getLinhas(); j++) {
                    CampoMinado.JButtonQuadrado botao = this.botao[i][j];
